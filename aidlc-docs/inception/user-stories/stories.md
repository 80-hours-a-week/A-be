...
## 에픽 12 — 온보딩 *(U14, 2026-07-23 편입)*

> 가입(및 기존 사용자 차기 로그인) 시 arXiv 카테고리 픽커로 관심사를 수집해 **관심사 설정 행동 이벤트**(FR-39 개정)로 U9 프로필을 시딩 — 개인화 콜드스타트 해소. 시드는 고정 반영이 아니며 이후 행동이 자연히 우세해진다(raw 90일 보관 = 자연 감쇠). **US-P5(키워드 부스트 라이브 소비, 에픽 8)는 본 트랙에 묶어 선행 실행**한다. 결정 기록: `requirements/onboarding.md`.

### US-OB1 — 가입 시 관심 카테고리 선택
**As** 연구자(P1), **I want** 가입할 때 arXiv 카테고리 픽커로 관심 분야를 고르기를, **so that** 첫 검색부터 내 관심사가 반영된다.
- **Given** 신규 사용자가 가입 플로우를 진행하면, **When** 픽커 단계에 도달하면, **Then** arXiv 카테고리 픽커가 노출되고 선택 결과가 시딩 입력(US-OB3)으로 전달된다.
- **Given** 사용자가 입력을 스킵하면, **When** 가입을 계속하면, **Then** 가입은 정상 완료되고 비개인화 경로로 동작한다.
- **Given** 픽커 데이터/저장이 실패하면, **When** 가입을 계속하면, **Then** 가입은 영향받지 않고 시딩만 저하 신호로 남는다.
- **Traces**: FR-44, NFR-P4, SEC-8

### US-OB2 — 기존 사용자 차기 로그인 프롬프트
**As** 연구자(P1), **I want** 기존 계정도 다음 로그인 때 관심사를 설정할 기회를 받기를, **so that** 온보딩 이전 가입자도 콜드스타트 해소 혜택을 받는다.
- **Given** 관심사 설정 이벤트가 없는 기존 사용자가, **When** 다음 로그인하면, **Then** 동일 카테고리 픽커가 프롬프트된다.
- **Given** 사용자가 스킵하면, **When** 로그인 흐름이 이어지면, **Then** 세션 진입이 차단되지 않고 프로필은 변경되지 않는다.
- **Traces**: FR-44, NFR-P4

### US-OB3 — 관심사 설정 이벤트로 프로필 시딩
**As** 연구자(P1), **I want** 내가 고른 관심사가 프로필에 시딩되기를, **so that** 행동 이력이 없어도 검색 개인화가 동작한다.
- **Given** 픽커 선택이 확정되면, **When** 기록되면, **Then** owner-scoped **관심사 설정 행동 이벤트**로 기록된다(직접 프로필 기록 금지 — C-7).
- **Given** 시딩 이벤트가 존재하면, **When** FR-19 집계가 실행되면, **Then** `categoryWeights`+`keywordWeights`가 비어 있지 않고 FR-20 적용 경로가 소비한다(카테고리=US-P4 라이브, 키워드=선행 US-P5).
- **Given** 사용자가 프로필 초기화/로그 삭제를 하면, **When** 재집계되면, **Then** 시딩 신호도 제거된다(QT-7).
- **Given** 실사용 행동이 쌓이거나 raw 이벤트가 90일을 넘으면, **When** 집계·보관 정책이 실행되면, **Then** 시드는 자연히 감쇠하고 행동 신호가 우세해진다.
- **Traces**: FR-45, FR-39, FR-19, FR-20, QT-7, NFR-P4, SEC-8

### US-OB4 — ORCID 관심사 유도
**As** 연구자(P1), **I want** ORCID로 로그인하면 내 프로필·저작에서 관심사가 제안되기를, **so that** 손으로 고르지 않아도 정확한 시드를 얻는다.
- **Given** ORCID 로그인 사용자가 온보딩에 진입하면, **When** ORCID 프로필·저작 조회가 성공하면, **Then** 유도된 관심사가 제안되고 **사용자 승인 후** 관심사 설정 이벤트로 기록된다(FR-39 개정).
- **Given** ORCID 조회가 실패하면, **When** 온보딩이 계속되면, **Then** 픽커-only로 저하되고 로그인/가입은 영향받지 않는다.
- **Given** 유도 관심사가 기록되면, **When** 저장되면, **Then** owner-scoped 비공개로 유지된다(SEC-8).
- **Traces**: FR-46, FR-45, NFR-P4, SEC-8
