...
## 에픽 13 — 트렌드/알림 *(U15, 2026-07-23 편입)*

> 명시적 팔로우 주제 목록(U14/U9 관심 프로필과 별개)을 설정 UI에서 관리하고, **옵트인한** 사용자에게 harvest 신규 논문을 키워드/임베딩 유사도로 매칭해 **plain list** 이메일 다이제스트(기본 daily, 사용자 설정)로 발송한다 — 구독제가 전제하는 리텐션 루프(C-10). 발송은 기존 `EMAIL_PROVIDER` 경로(SES on `559352512800`). ⚠️ **daily harvest는 일시 중지**(OQ-5 오너 결정) — 다이제스트 실효는 재개 결정에 종속, U15 빌드/테스트는 기존 코퍼스로 가능. 결정 기록: `requirements/trends-notifications.md`.

### US-TN1 — 팔로우 주제 목록 관리
**As** 연구자(P1), **I want** 관심 주제를 명시적으로 팔로우 목록에 담아 관리하기를, **so that** 어떤 주제의 신규 논문을 받아볼지 내가 직접 정한다.
- **Given** 로그인한 사용자가 설정 UI에 들어가면, **When** 팔로우 주제를 추가/수정/삭제하면, **Then** owner-scoped 팔로우 목록에 즉시 반영된다(SEC-8).
- **Given** 팔로우 주제가 등록되면, **When** 저장이 성공하면, **Then** 매칭용 임베딩이 준비된다(주제당 1회 — 기존 코퍼스 임베딩과 동일 공간).
- **Given** U14/U9 관심 프로필이 존재해도, **When** 팔로우 목록을 조회하면, **Then** 두 신호는 섞이지 않는다 — 팔로우 목록은 별개 명시 데이터다(OQ-1).
- **Traces**: FR-48, SEC-8
...
- **Given** 옵트인한 사용자와 팔로우 주제가 있으면, **When** 발송 주기(기본 daily, 설정 가능)가 되면, **Then** harvest 신규분 중 키워드/임베딩 유사도로 매칭된 논문의 **plain list**(제목·초록 링크, LLM 산출물 없음)가 기존 email 경로로 발송된다(C-9).
- **Given** 매칭되는 신규 논문이 없으면, **When** 발송 주기가 되면, **Then** 빈 다이제스트는 발송하지 않는다.
- **Given** 이메일 발송이 실패하면, **When** 다음 주기가 되면, **Then** 실패는 다른 사용자 발송·본 기능 외 경로에 영향을 주지 않는 비차단 저하로 처리된다.
- **Traces**: FR-47, C-9, C-1, C-6, NFR-C1(비대상 확인), SEC-8

## 에픽 14 — 웹검색 레퍼런스 *(U11 확장, 2026-07-23 편입)*

> evidence 에이전트(U11)에 **학술 공개 API**(Semantic Scholar·OpenAlex — 무키) 웹검색 도구를 추가해, corpus 근거를 보완하는 **웹 레퍼런스 목록**(링크백 전용 — claims 불참여, C-11)을 결과에 동봉한다. 같은 외부검색 포트는 novelty(U12)에도 노출. 신규 유닛 없음 — U11 확장. 호출은 기존 evidence 쿼터(30/day) 흡수, 별도 비용 없음(무키). 결정 기록: `requirements/web-references.md`.

### US-WR1 — 근거 결과에 웹 레퍼런스 동봉
**As** 연구자(P1), **I want** 근거형성 결과에 corpus 밖 학술 웹 레퍼런스(링크백)를 함께 받기를, **so that** corpus에 없는 관련 문헌으로 탐색을 넓힌다.
- **Given** evidence 턴이 성공 처리되면, **When** 학술 공개 API 검색이 결과를 반환하면, **Then** 별도 **웹 레퍼런스 목록**(제목·실재 URL/DOI 링크백)이 결과에 동봉된다 — claims의 supporting/conflicting에는 참여하지 않는다(C-11).
- **Given** 웹 레퍼런스가 표시되면, **When** 각 항목을 보면, **Then** 모든 URL/DOI는 프로바이더가 반환한 실재 값이다(무날조 — 조립·생성 금지, https·호스트 검증 통과분만).
- **Given** 외부 검색 질의가 만들어지면, **When** 프로바이더로 전송되면, **Then** 사용자 원문·Evidence 전체가 아닌 **최소 질의**만 나간다(카브아웃 Q9/Q25=A).
- **Given** 프로바이더 장애·미설정이면, **When** 턴이 처리되면, **Then** 웹 레퍼런스 없이 corpus-only 정상 결과가 반환된다(Noop 저하 — abstain·오류 아님).
- **Traces**: FR-49, C-11, SEC-9, SEC-5

### US-WR2 — novelty 외부 탐색에 학술 웹 합류
**As** 연구자(P1), **I want** novelty 에이전트의 외부 탐색이 학술 웹까지 커버하기를, **so that** 유사 연구 정리가 GitHub·데이터셋 너머의 문헌을 놓치지 않는다.
- **Given** novelty 잡이 외부 탐색을 수행하면, **When** 공유 외부검색 포트가 호출되면, **Then** 학술 웹 결과가 기존 GitHub/HuggingFace/Zenodo 결과와 같은 계약(RetrievalBundle)으로 합류한다.
- **Given** 학술 웹 검색이 실패하면, **When** 잡이 계속되면, **Then** 기존 외부 소스·잡 진행에 영향이 없다(소스별 비차단 저하 — novelty 기존 저하 계약).
- **Traces**: FR-49, C-11

## 에픽 15 — 구독제 *(U16, 2026-07-24 편입)*

> 2단 플랜(**free/plus**) — 유료 경계는 **에이전트 daily 쿼터 상향뿐**(기타 기능 전 티어 동일). **v1 결제 카브아웃**(C-12): ADMIN 수동 부여(월 기간)·paid-through-period 다운그레이드·만료 자동 free 전환. 플랜은 기존 CostGuard 집행에 쿼터 값만 공급(NFR-C1 계약 불변). ⚠️ plus 수치는 TBD — `reports/costguard-spend-report-2026-07.md`(OQ-5 선행물) 기준 설계 단계 오너 확정. 결정 기록: `requirements/subscription.md`.

### US-SB1 — 내 플랜 확인과 free 기본값
**As** 연구자(P1), **I want** 내 플랜과 오늘의 에이전트 쿼터를 확인하기를, **so that** 쓸 수 있는 호출량을 알고 계획한다.
- **Given** 플랜을 부여받지 않은 사용자가, **When** 서비스를 쓰면, **Then** free로 동작하고 현행 쿼터(evidence 30/day·novelty 5/day)가 그대로 적용된다(무회귀).
- **Given** 로그인 사용자가, **When** 플랜 정보를 조회하면, **Then** 본인 플랜·쿼터만 owner-scoped로 반환된다(SEC-8).
- **Given** 어떤 티어든, **When** 쿼터 외 기능(검색·요약·다이제스트 등)을 쓰면, **Then** 티어와 무관하게 동일하게 동작한다.
- **Traces**: FR-50, SEC-8, NFR-C1

### US-SB2 — plus 부여와 쿼터 상향
**As** 운영자(OP·ADMIN), **I want** 특정 사용자에게 plus를 월 기간으로 부여하기를, **so that** v1에서 결제 없이 상위 쿼터를 제공한다.
- **Given** ADMIN이, **When** plus를 부여하면, **Then** 월 단위 기간이 개시되고 대상 사용자의 에이전트 daily 쿼터가 plus 수치로 즉시 집행된다(기존 CostGuard 경로 재사용 — 집행 메커니즘 무변경).
- **Given** ADMIN이 아닌 사용자가, **When** 부여/회수를 시도하면, **Then** 거부된다(authz — UserRole.ADMIN 한정).
- **Given** 부여/회수가 처리되면, **When** 완료되면, **Then** 감사 로그가 남는다(SEC-14).
- **Traces**: FR-50, FR-51, C-12, SEC-14

### US-SB3 — 만료와 paid-through-period 다운그레이드
**As** 연구자(P1), **I want** 해지·미갱신 시에도 기간 말까지 plus를 유지하기를, **so that** 이미 부여된 기간의 가치를 잃지 않는다.
- **Given** plus 기간 중 회수(해지)가 발생하면, **When** 기간이 남아 있으면, **Then** 만료일까지 plus 쿼터가 유지된다(paid-through-period).
- **Given** plus 기간이 만료되면, **When** 갱신이 없으면, **Then** 수동 개입 없이 자동으로 free로 전환되고 **쿼터 값만** 변경된다 — 데이터·기능 접근은 그대로다.
...
| FR-47 (트렌드/알림 이메일 다이제스트) [U15] | US-TN2, US-TN3 |
| FR-48 (팔로우 주제 목록 관리) [U15] | US-TN1, US-TN2 |
