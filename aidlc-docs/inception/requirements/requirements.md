...
> **개정 (2026-07-23) — Phase 3 온보딩(U14) 등재**: `requirements/onboarding.md` Open Questions 7건 오너 결정 → **FR-44(가입 시 관심사 수집)·FR-45(U9 프로필 시딩)·FR-46(ORCID 관심사 유도)·C-7·C-8 등재**, **FR-39 개정**(관심사 설정을 의미 있는 행동 이벤트로 편입). 신규 유닛 **U14**, US-P5 선행 포함. 시딩은 이벤트 경로 전용(QT-7 불변식 유지).
> **개정 (2026-07-23) — Phase 3 트렌드/알림(U15) 등재**: `requirements/trends-notifications.md` Open Questions 8건 오너 결정 → **FR-47(이메일 다이제스트)·FR-48(팔로우 목록 관리)·C-9·C-10 등재**. 신규 유닛 **U15**. 옵트인 전용·plain list·기본 daily(설정 관리)·키워드/임베딩 매칭·SES on `559352512800`. **daily harvest 일시 중지**(OQ-5 오너 결정 — CDK `ArxivDailySchedule` `enabled=False`, 재개는 추후 논의) — 다이제스트 실효는 harvest 재개에 종속.
> **개정 (2026-07-23) — Phase 3 웹검색 레퍼런스(U11 확장) 등재**: `requirements/web-references.md` Open Questions 8건 오너 결정 → **FR-49(웹검색 레퍼런스 도구)·C-11 등재**. 신규 유닛 없음 — **U11 확장**, 외부검색 포트는 U12 novelty에도 노출. 학술 무키 API(Semantic Scholar·OpenAlex)·일반 웹(뉴스 버티컬 아님)·**링크백 전용**(claims 불참여)·본문 fetch 없음·기존 evidence 쿼터(30/day) 흡수·Noop 저하.
> **개정 (2026-07-24) — Phase 3 구독제(U16) 등재**: `requirements/subscription.md` Open Questions 8건 오너 결정 → **FR-50(플랜/티어 시스템)·FR-51(플랜 부여/만료 관리)·C-12 등재**. 신규 유닛 **U16**. **v1 결제 카브아웃**(OQ-1=b — PG 연동 없음, ADMIN 수동 부여) · 유료 경계=에이전트 쿼터 상향뿐 · 2단(free/plus) · 월 단위 기간 · paid-through-period 다운그레이드. **실행 게이트: plus 쿼터 수치는 CostGuard 실측 per-user spend 리포트 검토 후 확정**(그 전까지 TBD).
>
...
- **C-9** *(technical, 2026-07-23 — 트렌드/알림)* 신규 파이프라인 금지 — 다이제스트 논문 소스는 기존 daily harvest 산출물, 발송은 기존 email 경로(`EMAIL_PROVIDER` 심) 재사용. *(harvest는 오너 결정으로 일시 중지 — 재개 추후 논의.)*
