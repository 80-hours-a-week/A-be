...
> production-level completion of our goals. Tracking references: #251–#470 where cited below.
...
| 웹검색 레퍼런스 (고려) | ✅ Shipped (fork PR #6, 2026-07-23) | U11 extension: S2/OpenAlex link-back web references in evidence results (claims-isolated, BR-WR2) + novelty external-search join — merged to `develop` (`6835826`) with unit review APPROVE + NFR-P6 deadline fix |
...
| ✅ Closed; re-parse carried forward | Autoscale intentionally stays at **max=1** — restoring to 3 re-triggers the 2026-07-01 search-503 incident (arXiv ~1req/3s process-local limit); daily harvest (`docsuri-arxiv-daily` ENABLED) runs fine at 1. Queue-separation already implemented (dedicated docmodel queue+worker). Remaining #344 = native_html GROBID re-parse — a requester-pays corpus-quality backfill, deferred as non-hardening (confirm target with `migrate.py audit`). | #344 · #420 |
| ✅ LIVE (v1.15.0); #345 closed | Personalization shadow→real — machinery shipped v1.14 (**PR #441** env-gate + **PR #442** `apply_failed` counter), shadow review approved, then **PR #446 flipped the boost onto the live order** (`SEARCH_RERANK_LIVE`, v1.15.0, 2026-07-08). CDK pins the flag default `"true"`; rollback = `-c search_rerank_live=false`, no code. **PR #454** (v1.17.0) healed a silent no-op — profile TTL refresh + category backfill. US-P5 + keywordWeights carried forward. | #345 · #441 · #442 · #446 · #454 |
...
| 🪦 Moot | `mathieudutour/github-tag-action` node24 bump — was blocked on upstream release (rest of CI on node24 since PR #361); moot since 2026-07-10 — org repo frozen at v1.19.0, its CI retired with the off-AWS migration. | — |
...
> **Item 3 ✅ SHIPPED 2026-07-23**: stories(US-WR1~2) → design → implementation (S2/OpenAlex
> link-back tool, BR-WR2 LLM-isolation ordering, novelty port join, FE 웹 레퍼런스 section) →
> unit review APPROVE + NFR-P6 deadline fix — merged to `develop` via fork PR #6 (`6835826`).
